...
import 'package:app_bootsup/VistaCliente/detalleVinos/resultadoPago/PagosResultado.dart';
import 'package:app_bootsup/VistaCliente/detalleVinos/resultadoPago/error.dart';
import 'package:app_bootsup/VistaCliente/detalleVinos/resultadoPago/exitor.dart';
import 'package:app_bootsup/VistaCliente/detalleVinos/resultadoPago/pendiente.dart';
import 'package:app_bootsup/VistaCliente/screePrincipal/menuOpcionesPerfil/direccion.dart';
import 'package:app_bootsup/VistaCliente/screePrincipal/menuOpcionesPerfil/dni.dart';
...
class _CarritoPageState extends State<CarritoPageVinos> {
  ...
  Future<void> finalizarCompra(BuildContext context) async {
    try {
      ...
      final carritoService = Provider.of<CarritoServiceVinos>(
        context,
        listen: false,
      );
      ...
      final direccionEntrega = context
          .read<CarritoServiceVinos>()
          .direccionEntrega
          .trim();
      ...
      for (var producto in carrito) {
        ...
        if (producto['tallas'] != null && producto['tallas'] is Map) {
          cantidadTotal = (producto['tallas'] as Map).values.fold<int>(
            0,
            (prev, e) => prev + (e as int),
          );
        } else {
        ...
        if (imagenUrl != null) {
          ...
          if (response.statusCode == 200) {
            ...
            imagenStorageUrl = await storage
                .ref(storagePathCompra)
                .getDownloadURL();
          } else {
        ...
        if (productoSnapshot.docs.isNotEmpty) {
          ...
          await doc.reference.update({
            'cantidad': stockFinal < 0 ? 0 : stockFinal,
          });
        }
    ...
    try {
      ...
      if (estadoPago == 'approved') {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => const CompraFallidoScreenVinos()),
        );
      } else if (estadoPago == 'pending') {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => const CompraPendienteScreenVinos()),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        builder: (context, snapshot) {
          ...
          return StatefulBuilder(
            builder: (context, setState) {
              bool usarDireccionGuardada =
                  direccionUsuario.isNotEmpty &&
