...
class ComprasPageVinosC extends StatefulWidget {
  const ComprasPageVinosC({super.key});

  @override
  _ComprasPageState createState() => _ComprasPageState();
}

class _ComprasPageState extends State<ComprasPageVinosC> {
    ...
    if (criterio == 'rating') {
      final productosConRating = await Future.wait(
        snapshot.docs.map((doc) async {
          final comentariosSnap = await doc.reference
          ...
          return {'doc': doc, 'promedioRating': promedio};
        }),
      );

      productosConRating.sort(
        (a, b) => (b['promedioRating'] as double).compareTo(
          a['promedioRating'] as double,
        ),
      );
      ...
      final productosConComentarios = await Future.wait(
        snapshot.docs.map((doc) async {
          final comentarios = await doc.reference
              .collection('comentarios')
              .get();
          return {'doc': doc, 'comentarios': comentarios.size};
        }),
      );

      productosConComentarios.sort(
        (a, b) => (b['comentarios'] as int).compareTo(a['comentarios'] as int),
      );
  ...
  Future<void> _fetchProductos({String? categoria}) async {
    try {
      ...
      if (mounted) {
        final List<DocumentSnapshot> shuffled = List<DocumentSnapshot>.from(
  ...
  void _cargarMasProductos() {
    ...
    final siguienteProductos = _productos
        .skip(siguienteIndex)
